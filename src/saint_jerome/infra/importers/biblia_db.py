...
def load_source_books(
    *,
    ...
    source_url: str = DEFAULT_BIBLIA_JSON_URL,
    books_url: str = DEFAULT_LISTA_LIVROS_URL,
) -> list[SourceBook]:
    bible_payload = _load_json(source_file, source_url)
    books_payload = _load_json(books_file, books_url)
    return _build_source_books(bible_payload, books_payload)
...
def ensure_sqlite_schema(*, db_path: Path, project_root: Path) -> None:
...
def has_translation_sqlite(*, db_path: Path, translation_id: str) -> bool:
...
async def ensure_postgres_schema(*, database_url: str, project_root: Path) -> None:
    if asyncpg is None:
        raise RuntimeError(
            "O backend postgres requer a dependência 'asyncpg' instalada."
        )
...
async def has_translation_postgres(*, database_url: str, translation_id: str) -> bool:
    if asyncpg is None:
        raise RuntimeError(
            "O backend postgres requer a dependência 'asyncpg' instalada."
        )
    ...
    try:
        ...
        translation_row = await connection.fetchrow(
            "SELECT 1 FROM translations WHERE id = $1 LIMIT 1",
            translation_id,
        )
        return translation_row is not None
    finally:
